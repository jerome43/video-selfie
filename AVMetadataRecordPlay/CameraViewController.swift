...
class CameraViewController: UIViewController, AVCaptureFileOutputRecordingDelegate, CLLocationManagerDelegate {
	...
	private func configureSession() {
		...
		if (session.canSetSessionPreset(AVCaptureSessionPreset640x480)) {
			session.sessionPreset = AVCaptureSessionPreset640x480 // change résolution (default : 1280x720)
			}
		
		// Add video input.
		do {
			var defaultVideoDevice: AVCaptureDevice?
			// desactivation pour utilier la caméra frontale par défaut
			/*
	...
	/*
	// pour changer de caméra (back / front) => plus nécessaire
...
*/
		...
		*/
		...
		sessionQueue.async { [unowned self] in
			if !self.movieFileOutput.isRecording {
				...
				// limitation de la durée de la vidéo à 20 s
				self.movieFileOutput.maxRecordedDuration = CMTime.init(seconds: 20, preferredTimescale: 30);
	...
	func capture(_ captureOutput: AVCaptureFileOutput!, didFinishRecordingToOutputFileAt outputFileURL: URL!, fromConnections connections: [Any]!, error: Error!) {
		...
		*/
		...
		DispatchQueue.main.async {
			...
			self.recordButton.setTitle(NSLocalizedString("Enregistrez votre vidéo - Start your video !", comment: "Recording button record title"), for: [])
		}
	}
	...
	private func presentPlayer() {
		...
		let presentPlayerViewController = storyboard.instantiateViewController(withIdentifier: "player") as! PlayerViewController
		presentPlayerViewController.outputFileURL = self.outputFileURL! // pass the path of the video filen
		self.navigationController?.pushViewController(presentPlayerViewController, animated: true)
